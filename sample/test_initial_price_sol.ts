...
const main = async () => {
  ...
  const donation = getMemeDonationDestinationFromName(
    MemeDonationDestinationName.OGMeme,
  );

  const metadataUri =
    'https://goodr-fun.s3.us-east-1.amazonaws.com/metadata/test-initial-price-SOL';

  console.log('=== SOL Initial Price Test (Zero Buy) ===');
  ...
  const result = await sdk.createAndBuy(wallet, {
    mint: mint,
    buySolAmount: new BigNumber(0), // Zero initial buy
    slippageBasisPoints: 500,
    meme: donation.name,
    metadata: {
      name: 'TEST_SOL_INITIAL',
      symbol: 'TSOL',
  ...
  console.log(
    `📈 Initial Price: ${initialState.priceData.price.toFixed(16)} SOL per token`,
  );
  console.log(
    `💎 Initial Market Cap: ${initialState.priceData.marketCap.toFixed(4)} SOL`,
  );
  console.log(
  ...
  console.log(
    `📈 Price: ${priceDataFromMethod.price.toFixed(16)} SOL per token`,
  );
  console.log(`💎 Market Cap: ${priceDataFromMethod.marketCap.toFixed(4)} SOL`);
  console.log(
  ...
  console.log('\n=== COMPARISON ===');
  console.log(`Expected SOL price: ~0.0000001667 SOL per token`);
  console.log(
    `Actual SOL price:   ${initialState.priceData.price.toFixed(16)} SOL per token`,
  );

  const expectedPrice = new BigNumber('0.0000001667');
